...
class StatsBar: # Class for all bars drawn
    def __init__(self, fillColour, bgColour, x, y, surface, barLength, barHeight, barThickness, param1, param2, barType, textX, textY, drawText):
    ...
    def draw(self):
        ...
        outlineRect = pg.Rect(self.xPos - self.barThickness, self.yPos - self.barThickness, self.barLength + 2 * self.barThickness, self.barHeight + 2 * self.barThickness)
        pg.draw.rect(self.surface, self.bgColour, backgroundRect)
        pg.draw.rect(self.surface, self.fillColour, fillRect)
        pg.draw.rect(self.surface, WHITE, outlineRect, width = self.barThickness)
...
class Game: # Main class that initialises everything required for the game, manages the main game loop and renders all sprites
    def __init__ (self): 
        pg.init()
        pg.mixer.init()
    ...
    def render(self): 
        ...
        for enemy in self.enemies:
            self.miniBar = StatsBar(PURPLE, BLACK, enemy.rect.left + enemy.rect.width // 4, enemy.rect.top - 25, self.screen, enemy.rect.width // 2, 10, 1, enemy.currentHP, enemy.maxHP, "HP", 0, 0, False)
            self.miniBar.draw()
        self.drawWaveDisplay()
    ...
    def drawPlayerStats(self):
        ...
        self.playerHealthBar = StatsBar(GREEN, RED, 20, 60, self.playerStatsBoxSurface, 400, 20, 5, self.player.currentHP, self.player.maxHP, "HP", 220, 62, True)
        self.playerHealthBar.draw()
        self.playerEXPBar = StatsBar(BLUE, BLACK, 20, 120, self.playerStatsBoxSurface, 400, 20, 5, self.player.currentEXP, self.player.maxExp, "EXP", 220, 122, True)
    ...
    def drawEnemyStats(self):
        ...
        for enemy in self.enemies:
            if enemy.rect.collidepoint(mouseLocation):
                ...
                self.enemyHealthBar = StatsBar(PURPLE, BLACK, 20, 100, self.enemyStatsBoxSurface, 400, 20, 5, enemy.currentHP, enemy.maxHP, "HP", 220, 102, True)
                self.enemyHealthBar.draw()

    def drawWaveDisplay(self):
        '''Method that draws wave display'''
        self.waveBarBox = pg.Rect(30, 30, 160, 35)
        self.waveBarBoxSurface = self.screen.subsurface(self.waveBarBox)
        self.waveBarBoxSurface.fill(BLACK)
        self.waveBarRect = pg.Rect(25, 25, 170, 45)
        waveString = "Wave " + str(self.currentWave)
        drawText(self.waveBarBoxSurface, waveString, WHITE, 40, True, 80, 5)
        pg.draw.rect(self.screen, WHITE, self.waveBarRect, 5)
